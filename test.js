...
const client = new MongoClient('mongodb+srv://vidhyasagar:<asdfghjkl>@cluster0.lt9nhdm.mongodb.net/', { useNewUrlParser: true });
...
async function updateExpiry(shortURL, daysToAdd) {
  ...
  const result = await db.collection('url_mappings').findOneAndUpdate(
    { shortURL },
    { $set: { expiryDate: new Date(new Date().getTime() + daysToAdd * 86400000) } }, // 86400000 ms in a day
  );
